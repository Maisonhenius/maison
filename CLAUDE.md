...
- **Hero video is a single looping brand film with dual-codec sources**: `index.html` has two `<source>` elements inside the hero `<video loop muted autoplay>` — WebM (VP9 + Opus) listed first for Chrome/Firefox/Edge, MP4 (H.264 + AAC) fallback for Safari. Browser picks the first supported source. Sound toggle flips `heroVideo.muted`, NOT a separate `<audio>` element. No `heroVideos` array, no `ended`-event cycle, no `#bgMusic`. Source file (`assets/videos/Brand film.mp4`, 124 MB) is gitignored; encoded `web/brand-film.{webm,mp4}` are tracked.

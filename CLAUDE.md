...
- **Magic-link redirect allowlist (silent-fail gotcha)**: `send-link` / forgot-password build `redirect_to` from the live `host` header, so EVERY domain that serves auth (canonical is `www.maisonhenius.com`) must have its callback paths on the Supabase Redirect URLs allowlist (Auth > URL Configuration). A `redirect_to` that isn't on the list does NOT error: Supabase silently falls back to the Site URL, so the magic link dumps the user on the home page instead of `/admin/auth/callback`. Fixed with wildcards `https://www.maisonhenius.com/**` + `https://maisonhenius.com/**`; Site URL set to `https://www.maisonhenius.com/`.
...
- **`psql` is NOT installed locally**. Schema migrations run via `cd server && python migrations/run_migration.py <name.sql>` (psycopg2 + DATABASE_URL from `.env.local`). Data migrations are standalone Python scripts in the same directory using `supabase-py` with the service role key — see `002_seed_products.py`, `008_migrate_product_media_to_storage.py` as patterns. **The Supabase MCP server is NOT connected to the Maison project** (the linked account only exposes unrelated `halulu` projects), so for ad-hoc DB reads/writes use a `psycopg2` + `DATABASE_URL` Python one-liner mirroring `run_migration.py`, never the `mcp__*Supabase*` tools.
